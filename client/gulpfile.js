...
var config;
try {
  config = require('./build.cfg.json');
} catch (error) {
  if (error.code === 'MODULE_NOT_FOUND') {
    console.warn('Please create a ./build.cfg.json before using gulp. A sample file is available as ./build.cfg.sample.json. For now the sample file configuration is used to configure build parameters.');
    config = require('./build.cfg.sample.json');
  } else {
    throw error;
  }
}
...
gulp.task('serve', function() {
  ...
  var middlewares = [];
  for (var sourceUri in serverConfig.proxies) {
    var targetUri = serverConfig.proxies[sourceUri];
    var proxy = proxyMiddleware(sourceUri, {target: targetUri});
